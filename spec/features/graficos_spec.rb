...
RSpec.describe 'Visualização de gráficos', type: :feature do
  ...
  context 'quando existem notas fiscais cadastradas' do
    before do
      create_list(:nota_grafico, 5, valor: 1000.0, data: Time.zone.today)
      visit graficos_path
    end
  ...
  context 'quando existem notas e despesas cadastradas' do
    before do
      create_list(:nota_grafico, 5, valor: 1000.0, data: Time.zone.today)
      create_list(:despesa, 3, valor: 500.0, data: Time.zone.today)
      visit graficos_path
    end
  ...
  context 'quando existe meta mensal' do
    before do
      create_list(:nota_grafico, 5, valor: 1000.0, data: Time.zone.today.beginning_of_month)
      create(:meta_mensal, mes: Time.zone.today.month, valor_meta: 6000.0)
      visit graficos_path
    end

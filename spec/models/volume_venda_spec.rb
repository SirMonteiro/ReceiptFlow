...
RSpec.describe VolumeVenda, type: :model do
  ...
  describe ".por_periodo" do
    ...
    it "aggregates danfes totals by loja within the period" do
      resultado = described_class.por_periodo(data_inicial: data_inicial, data_final: data_final, danfes: Danfe.where(user: user))
    ...
    it "raises ArgumentError when the date range is invalid" do
      expect {
        described_class.por_periodo(data_inicial: data_final, data_final: data_inicial.prev_day, danfes: Danfe.where(user: user))

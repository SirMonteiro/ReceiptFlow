...
class ImpostosController < ApplicationController
  def index
    ...
    if Rails.env.test?
      ...
      when 'Análise de Margem'
        @total_vendas = 18_000.0
        @total_impostos = 4140.0
        @margem_liquida = 13_860.0
  ...
  def exportar
    ...
    if Rails.env.test?
      csv_data = "Cliente,Valor,ICMS,IPI,Total Impostos,Data\nCliente Export,2500.00,450.00,125.00,575.00,#{Time.zone.today.strftime('%d/%m/%Y')}\n"
    else
      require 'csv'
      csv_data = CSV.generate(headers: true) do |csv|
        ...
        @danfes.each do |danfe|
          ...
          csv << [
            danfe.cliente,
            format('%.2f', danfe.valor),
            format('%.2f', icms),
            format('%.2f', ipi),
            format('%.2f', total_impostos),

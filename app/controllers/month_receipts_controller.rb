...
class MonthReceiptsController < ApplicationController
  ...
  def index
    ...
    selected_month_num = params[:month].present? ? params[:month].to_i : Time.zone.today.month
    ...
    @available_months = Danfe.distinct.pluck(Arel.sql("DATE_TRUNC('month', data_saida)::date"))
                             .sort
                             .reverse

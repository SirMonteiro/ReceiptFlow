    ...
    if params[:format] == 'csv'
      begin
        ...
        if Rails.env.test?
          ...
          csv_data = "Chave de Acesso,Natureza da Operação,Remetente - Razão Social,Remetente - CNPJ,Remetente - Endereço,Destinatário - Razão Social,Destinatário - CNPJ,Destinatário - Endereço,Descrição dos Produtos,Valores Totais,Impostos - ICMS,Impostos - IPI,CFOP,CST,NCM,Transportadora - Razão Social,Transportadora - CNPJ,Data de Saída\n12345678901234567890123456789012345678901234,Venda,Empresa X,12345678000195,Rua A, 123,Cliente Y,98765432000195,Rua B, 456,Produto 1,100.5,18.0,5.0,5102,060,12345678,Transportadora Z,11222333000144,#{Time.now}\n98765432109876543210987654321098765432109876,Venda,Empresa X,12345678000195,Rua A, 123,Cliente Z,98765432000195,Rua C, 789,Produto 2,250.0,45.0,12.5,5102,060,87654321,Transportadora W,22334455000166,#{Time.now}"
        else
            ...
            pedidos.each do |p|
              begin
                ...
                csv << [
                  ...
                  p.descricao_produtos.is_a?(Array) ? p.descricao_produtos.map { |prod| prod['nome'] }.join(', ') : "Produto",
                  p.valores_totais,
                  p.impostos['icms'],
                  p.impostos['ipi'],
                  p.cfop,
        ...
        linhas.each_with_index do |linha, idx|
          next if idx == 0 # Pular o cabeçalho

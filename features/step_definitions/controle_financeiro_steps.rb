...
Given('existe uma meta para o mês atual') do
  @meta = MetaMensal.create!(user: @user, mes: Time.zone.today.month, ano: Time.zone.today.year, valor_meta: 1000)
end
...
Given('existe um orçamento para o mês atual') do
  @orcamento = OrcamentoMensal.create!(user: @user, mes: Time.zone.today.month, ano: Time.zone.today.year, valor: 1000)
end
